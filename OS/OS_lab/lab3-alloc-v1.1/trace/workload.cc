...
char* gen_random_string(int length)
{
	int flag, i;
	char* string;
    // struct timeval cur_malloc_time;
    // gettimeofday(&cur_malloc_time, NULL);
    // long sec1=cur_malloc_time.tv_sec,usec1=cur_malloc_time.tv_usec;
    ...
    // gettimeofday(&cur_malloc_time, NULL);
    // long sec2=cur_malloc_time.tv_sec,usec2=cur_malloc_time.tv_usec;
...
void* workload_run(void *workload){
    #ifdef _OUTFILE_
    std::ofstream first_file;
    //std::ofstream best_file;
    first_file.open("first.csv",std::ios::out);
    //best_file.open("best.csv",std::ios::out);
    first_file<<"before free"<<","<<"after free"<<","<<"time"<<std::endl;
    //best_file<<"before free"<<","<<"after free"<<","<<"time"<<std::endl;
    #endif 
    struct timeval cur_time;
    //printf("workload_run_begin()\n");
    for(int loop=0; loop<LOOP_NUM; loop++){
        ...
        auto before_free=get_utilization();
        std::cout<<"before free: "<<before_free<<std::endl;
        workload_delete((struct workload_base*)workload);
        auto after_free=get_utilization();
        std::cout<<"after free: "<<after_free<<std::endl;
        long sec2=cur_time.tv_sec,usec2=cur_time.tv_usec;
        auto malloc_time=(sec2-sec1)*1000 + (usec2-usec1)/1000;
        std::cout<<"malloc time of loop "<< loop <<" : "<<malloc_time << "ms" << std::endl;        
        #ifdef _OUTFILE_
        first_file<<before_free<<","<<after_free<<","<<malloc_time<<std::endl;
        #endif
    }
}
