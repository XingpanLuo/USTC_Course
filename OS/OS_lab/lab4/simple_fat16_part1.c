                  ...
                  off_t offset, struct fuse_file_info *fi)
{
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      if((Root.DIR_Attr==16||Root.DIR_Attr==32)&&Root.DIR_Name[0]!=0xe5){
  ...
  else  // 要查询的目录不是根目录
  {
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      if((Dir.DIR_Attr==16||Dir.DIR_Attr==32)&&Dir.DIR_Name[0]!=0xe5){
               ...
               struct fuse_file_info *fi)
{
  ...
  /*** BEGIN ***/
  off_t beginBytes=offset;
  off_t endBytes=offset+size-1;
  int sec_per_clus=fat16_ins->Bpb.BPB_SecPerClus;//每个簇有几个扇区
  int beginBytes_in_cluster=beginBytes/(BYTES_PER_SECTOR*sec_per_clus);//begin在第几个簇
  int endBytes_in_cluster=endBytes/(BYTES_PER_SECTOR*sec_per_clus);//end在第几个簇
  int beginBytes_in_sector=(beginBytes%(BYTES_PER_SECTOR))%sec_per_clus;//begin在簇的第几个扇区
  int endBytes_in_sector=(endBytes%(BYTES_PER_SECTOR))%sec_per_clus;//end在簇的第几个扇区
  int beginBytes_in_location=beginBytes%BYTES_PER_SECTOR;//begin在扇区的哪个位置
  int endBytes_in_location=endBytes%BYTES_PER_SECTOR;//end在扇区的哪个位置
  ...
  BYTE sector_buffer[BYTES_PER_SECTOR];
  ClusterN = Dir.DIR_FstClusLO; //目录项中存储了我们要读取的第一个簇的簇号
  for(int i=0;i<beginBytes_in_cluster;i++){
    ClusterN=fat_entry_by_cluster(fat16_ins,ClusterN);
  }
  beginBytes_in_cluster=ClusterN;//找到真正的起始簇号
  ClusterN = Dir.DIR_FstClusLO; 
  for(int i=0;i<endBytes_in_cluster;i++){
    ClusterN=fat_entry_by_cluster(fat16_ins,ClusterN);
  }
  endBytes_in_cluster=ClusterN;//找到真正的终止簇号
  /*第一簇的第一扇区 */
  first_sector_by_cluster(fat16_ins, beginBytes_in_cluster, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
  sector_read(fat16_ins->fd, FirstSectorofCluster+beginBytes_in_sector, sector_buffer);
  off_t buffer_write_size=0;
  WORD OffRemainSize=BYTES_PER_SECTOR;
  if(beginBytes_in_location!=0)
    OffRemainSize= (BYTES_PER_SECTOR - beginBytes_in_location) % BYTES_PER_SECTOR;
  memcpy(buffer+buffer_write_size,sector_buffer+beginBytes_in_location,OffRemainSize);
  buffer_write_size+=OffRemainSize;
  /*第一簇的其余扇区 */
  WORD SectorN=beginBytes_in_sector+1;
  while (SectorN<=sec_per_clus-1)
  {
    sector_read(fat16_ins->fd,FirstSectorofCluster+SectorN,sector_buffer);
    memcpy(buffer+buffer_write_size,sector_buffer,BYTES_PER_SECTOR);
    buffer_write_size+=BYTES_PER_SECTOR;
    SectorN++;
  }
  /*除去首尾，中间所有簇的所有扇区 */
  ClusterN = fat_entry_by_cluster(fat16_ins, beginBytes_in_cluster);
  // printf("ClusterN:%ld\n",ClusterN);
  // printf("endBytes_in_cluster:%ld\n",endBytes_in_cluster);
  while(ClusterN<endBytes_in_cluster){
    first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
    if (FatClusEntryVal == 0xffff){
      return buffer_write_size;//文件结束
    }
  ...
  /*最后一簇的规则扇区 */
  first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
  //printf("endBytes_in_sector:%ld\n",endBytes_in_sector);
  for(SectorN=0;SectorN<endBytes_in_sector;SectorN++){
    sector_read(fat16_ins->fd,FirstSectorofCluster+SectorN,sector_buffer);
    memcpy(buffer+buffer_write_size,sector_buffer,BYTES_PER_SECTOR);
    buffer_write_size+=BYTES_PER_SECTOR;
  }
  /*最后一簇的最后一个扇区 */
  if(SectorN<sec_per_clus){
    sector_read(fat16_ins->fd,FirstSectorofCluster+SectorN,sector_buffer);
    memcpy(buffer+buffer_write_size,sector_buffer,endBytes_in_location+1);
    buffer_write_size+=endBytes_in_location+1;
    // printf("endBytes_in_location:%ld\n",endBytes_in_location+1);
    // printf("buffer5:%s\n",buffer);
  }
  /*** END ***/
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  char *nfile_name=paths[pathDepth-1];
  printf("nfile_name:%s\n",nfile_name);
  /** TODO:
  ...
  if (strcmp(prtPath, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      else if(strcmp(Root.DIR_Name,nfile_name)==0){
        break;
      }
      if (i % 16 == 0 && i != fat16_ins->Bpb.BPB_RootEntCnt)
      {
  ...
  else
  {
    ...
    do
    {
      first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
      DirSecCnt = 1;
      int DIR_PER_Clus=fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec / BYTES_PER_DIR;
      for(i = 1;i <= DIR_PER_Clus ; i++)
      {
        memcpy(&Dir, &sector_buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);
        ...
        else if(strcmp(Dir.DIR_Name,nfile_name)==0){//重名
          break;
        }

        if (i % 16 == 0 && i != DIR_PER_Clus)
        {
          sector_read(fat16_ins->fd, FirstSectorofCluster + DirSecCnt, sector_buffer);
          DirSecCnt++;
        }
    ...
    //}while(ClusterN <= 0xFFEF && ClusterN >= 0x0002 && findFlag == 0&&ClusterN!=0xFFFF);
    }while(findFlag == 0&&ClusterN!=0xFFFF);
    /*** END ***/
  }
  ...
  if (findFlag == 1) {
    ...
    /*** BEGIN ***/
    dir_entry_create(fat16_ins,sectorNum,offset,nfile_name,0x20,0xffff,0);
    /*** END ***/
  }
  return 0;
}
...
int dir_entry_create(FAT16 *fat16_ins, int sectorNum, int offset, char *Name, BYTE attr, WORD firstClusterNum, DWORD fileSize)
{
  ...
  fseek(fd, BYTES_PER_SECTOR * sectorNum+offset, SEEK_SET);
  fwrite(entry_info,sizeof(BYTE),BYTES_PER_DIR,fd);
  fflush(fd);
  /*** END ***/
  free(entry_info);
  return 0;
...
//FatClusEntryVal是对应簇的FAT表项，FirstSectorofCluster是对应簇的文件数据域的第一个sector,buffer中是对应簇的后512Byte大小的缓存
   ...
   **/
  /*** BEGIN ***/
  BYTE *FAT_temp = malloc(2 * sizeof(BYTE));
  memset(FAT_temp, 0 ,2 * sizeof(BYTE));

  fseek(fd, fat16_ins->Bpb.BPB_RsvdSecCnt * fat16_ins->Bpb.BPB_BytsPerSec + ClusterNum * 2 ,SEEK_SET);
  fwrite(FAT_temp, 2 * sizeof(BYTE), 1, fd);
