...
int write_fat_entry(FAT16 *fat16_ins, WORD ClusterN, WORD data)
{
  ...
  uint FirstFatSecNum = fat16_ins->Bpb.BPB_RsvdSecCnt;                                // 第一个FAT表开始的扇区号=保留扇区数
...
int fat16_mkdir(const char *path, mode_t mode)
{
  ...
  if (strcmp(prtPath, "/") == 0)
  {
    ...
    for (i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      if ((Root.DIR_Name[0] == 0xe5 || Root.DIR_Name[0] == 0x00) && Root.DIR_Attr != 0x0f)
      {
        findFlag = 1;
  ...
  else
  {
    ...
    do
    {
      ...
      for (i = 1; i <= DIR_PER_Clus; i++)
      {
        ...
        if ((Dir.DIR_Name[0] == 0x00 || Dir.DIR_Name[0] == 0xe5) && Dir.DIR_Attr != 0x0f)
        {
  ...
  if (findFlag == 1)
  {
    ...
    // first_sector_by_cluster()中有簇的第一个扇区的计算方法。
...
void dir_entry_delete(FAT16 *fat16_ins, off_t offset)
{
  ...
  fwrite(&tmp, sizeof(BYTE), 1, fat16_ins->fd);
  fflush(fat16_ins->fd);
  /*** END ***/
}
...
int fat16_rmdir(const char *path)
{
  ...
  while (ClusterNum >= 0x2 && ClusterNum < 0xffff)
  {
...
size_t write_to_cluster_at_offset(FAT16 *fat16_ins, WORD clusterN, off_t offset, const BYTE *data, size_t size)
{
  ...
  { //一个扇区就可以写完。
    //sector_write(fat16_ins->fd,FirstSectorofCluster,(const char *)data);
    fwrite((const char *)data, sizeof(char), size, fat16_ins->fd);
    fflush(fat16_ins->fd);
    return size;
  }
  else //一个扇区不够，先把这个扇区写满
  {
  ...
  fseek(fd, fat16_ins->Bpb.BPB_BytsPerSec - offset, SEEK_SET);
  int i=0;
  while (size_cp > fat16_ins->Bpb.BPB_BytsPerSec)
  { //整个扇区整个扇区的写入，直到剩余数据不够写满一个扇区
    fwrite((const char *)data, sizeof(char), BYTES_PER_SECTOR, fd);
    data += BYTES_PER_SECTOR;
    size_cp -= BYTES_PER_SECTOR;
    i++;
    fseek(fd, BYTES_PER_SECTOR * i, SEEK_SET);
  }
  //剩余不够一个扇区的数据写入。
  fwrite((const char *)data, sizeof(char), size_cp, fd);
  fflush(fat16_ins->fd);
  fflush(fd);
  // fseek(fat16_ins->fd, FirstSectorofCluster * fat16_ins->Bpb.BPB_BytsPerSec + offset, SEEK_SET);
  // fwrite((const char *)data, sizeof(char), size, fat16_ins->fd);
...
WORD file_last_cluster(FAT16 *fat16_ins, DIR_ENTRY *Dir, int64_t *count)
{
  ...
  while (is_cluster_inuse(ClusterPre))
  {
    ...
    if (!is_cluster_inuse(ClusterNxt))
      cur = ClusterPre; //下一个簇无效，则这个簇就是最后一个簇
...
int write_file(FAT16 *fat16_ins, DIR_ENTRY *Dir, off_t offset_dir, const void *buff, off_t offset, size_t length)
{
  ...
  int cluster_all = (length + offset - 1) / bytes_per_clus + 1;
  int cluster_req = cluster_all - count; //总的-已有的=剩余的
  ...
  //从ClusterN开始
  if (off_tmp + length <= fat16_ins->ClusterSize) //一个簇就够了
  {
...
int fat16_truncate(const char *path, off_t size)
{
  ...
  else if (old_size < new_size)
  {
    ...
    memset(tmp, 0, bytes_per_clus);
    //最后一个簇的off到结尾写为0
    write_to_cluster_at_offset(fat16_ins, last_cluster, last_off, (const BYTE *)tmp, bytes_per_clus - last_off);
    //增加新簇
    WORD last_clus;
    int64_t cluster_req = new_cluster_count - cur_cluster_count; //需要增加几个簇
    if (cluster_req > 0)
    {
  ...
  { // 截断文件
    ...
    WORD ClusterN = Dir.DIR_FstClusLO;
    int cnt = 0;
    while (cnt < new_cluster_count - 1) //新的文件有5个簇，则簇号为4的簇的值为0xffff，所以这里是new_cluster_count-1
    {
      ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
      cnt++;
    }
    ...
    ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
    while (ClusterN > 1 && ClusterN < 0xffff)
    {
      free_cluster(fat16_ins->fd,ClusterN);
      // ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
      // int bytes_per_clus = fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec;
      // char tmp[bytes_per_clus];
      // memset(tmp, 0, bytes_per_clus);
      // write_to_cluster_at_offset(fat16_ins, ClusterN, 0, (const BYTE *)tmp, bytes_per_clus);
    }
    /*** END ***/
  }
