...
int process_redirect(int argc, char **argv, int *fd)
{
    ...
    while (i < argc)
    {
        ...
        if (strcmp(argv[i], ">") == 0)
        {
            // TODO: 打开输出文件从头写入
            /*
            O_RDWR:可读可写；O_WRONLY 只可写；O_RDONLY 只可读
            O_TRUNC 清空原来的内容 O_APPEND 不清空原来的内容，而是追加
            O_CREAT 如果没有该文件，就创建该文件
            */
            if (argv[i + 1] == "\0")
                printf("bash: 未预期的符号“newline”附近有语法错误\n");
                //tfd = open("./out.txt", O_RDWR | O_CREAT | O_TRUNC, 0666);
            else
        ...
        else if (strcmp(argv[i], ">>") == 0)
        {
            // TODO: 打开输出文件追加写入
            if (argv[i + 1] == "\0")
                //tfd = open("./out.txt", O_WRONLY | O_APPEND | O_CREAT, 0666);
                printf("bash: 未预期的符号“newline”附近有语法错误\n");
            else
                tfd = open(argv[i + 1], O_WRONLY | O_APPEND, 0666);
            if (tfd < 0)
            {
                printf("open '%s' error: %s\n", argv[i + 1], strerror(errno));
            }
        ...
        else if (strcmp(argv[i], "<") == 0)
        {
            // TODO: 读输入文件
            if (argv[i + 1] == "\0")
                //tfd = open("./in.txt", O_RDONLY | O_CREAT | O_TRUNC, 0666);
                printf("bash: 未预期的符号“newline”附近有语法错误\n");
            else
                tfd = open(argv[i + 1], O_RDONLY | O_CREAT, 0666);
            if (tfd < 0)
            {
                printf("open '%s' error: %s\n", argv[i + 1], strerror(errno));
            }
...
int main()
{
    ...
    char *cmdlines[MAX_CMDLINE_LENGTH];
    char *commands[128];
    int cmd_count;
    char delim[] = " "; //分割符
    int line_count = 0;
    ...
    while (1)
    {
        ...
        char cur_dir_buf[30];
        getcwd(cur_dir_buf, 30);
        if (line_count == line_num )
        {
            printf("{lxp}shell:[%s]-> ", cur_dir_buf);
            fflush(stdout);

            fgets(cmdline, 256, stdin);
            strtok(cmdline, "\n");

            line_num = split_string(cmdline, ";", cmdlines);
            line_count=0;
        }

        /* TODO: 基于";"的多命令执行，请自行选择位置添加 */
        strcpy(cmdline, cmdlines[line_count]);
        line_count++;
        ...
        { // 没有管道的单一命令
            ...
            int argc;
            int fd[2];
            char command[MAX_CMDLINE_LENGTH];
            ...
            {
                argc = split_string(command, " ", argv);
#ifdef _DEBUG_
                printf("%d %s\n", argc, argv[0]);
#endif
            }
