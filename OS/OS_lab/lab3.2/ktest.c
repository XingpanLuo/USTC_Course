...
// 定义要输出的文件
#define OUTPUT_FILE "/home/lumos/oslab/lab3/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffb26b0780;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffb2694160;
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    //struct pgt_t mpgd=mm->pgd;
    pgd_t *mpgd=NULL;
    pud_t *mpud=NULL;
    pmd_t *mpmd=NULL;
    pte_t *mpte=NULL;
    mpgd=pgd_offset(mm,virt);
    if(!(pgd_none(*mpgd)&&pgd_bad(*mpgd))){
        mpud=pud_offset((p4d_t *)mpgd,virt);
        if(!(pud_none(*mpud)&&pud_bad(*mpud))){
            mpmd=pmd_offset(mpud,virt);
            if(!(pmd_none(*mpmd)&&pmd_bad(*mpmd))){
                mpte=pte_offset_kernel(mpmd,virt);
                if(!(pte_none(*mpte))){
                    return pte_pfn(*mpte);
                }
            }
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        while(vma!=NULL){
            ...
            while(virt_addr!=vma->vm_end){
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
                unsigned long phys=virt2phys(mm,virt_addr);
                unsigned long tphys=page_to_pfn(page);
                if(phys!=tphys&&phys!=-1)
                    printk("error:phys==tphys:%d,%d\n",phys,tphys);
                // else
                //     printk("%d",1);
                record_two_data(virt_addr,phys);
                virt_addr+=PAGE_SIZE;
            }
            vma=vma->vm_next;
        }
...
// }
    ...
    unsigned long start_data,end_data,start_code,end_code,data,code;
    ...
    // printk("start_data:%ld",start_data);
    // printk("end_data:%ld",end_data);
    for(data=start_data;data<=end_data;data+=PAGE_SIZE){
        vma=find_vma(mm,data);
        if(vma!=NULL){
            unsigned long virt_addr=vma->vm_start;
            for(;virt_addr<=vma->vm_end;virt_addr+=PAGE_SIZE){
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
                if(!IS_ERR_OR_NULL(page)){
                    void *addr=kmap_atomic(page);
                    int len=PAGE_SIZE;//int len=strlen(addr)    这里void* 不能用strlen()
                    kernel_write(log_file, (char *)addr, len, &log_file->f_pos);
                    kunmap_atomic(addr);
                }
            }
        }
    ...
    for(code=start_code;code<=end_code;code+=PAGE_SIZE){
        vma=find_vma(mm,code);
        if(vma!=NULL){
            unsigned long virt_addr=vma->vm_start;
            for(;virt_addr<=vma->vm_end;virt_addr+=PAGE_SIZE){
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
    ...
    //尝试的二种遍历方式
    // struct vm_area_struct *vma_start=find_vma(mm,start_data);
    // struct vm_area_struct *vma_end=find_vma(mm,end_data);
    // for(vma=vma_start;vma<vma_end&&vma!=NULL;vma=vma->vm_next){
    //     printk("%p\n",vma);
    //     if(vma!=NULL){
    //         unsigned long virt_addr=vma->vm_start;
    //         for(;virt_addr<=vma->vm_end;virt_addr+=PAGE_SIZE){
    ...
    // for(data=start_data;data<=end_data;data+=PAGE_SIZE){
    //     vma=find_vma(mm,data);
    //     struct vm_area_struct *save_vma=vma;
    //     while(vma!=NULL){
    //         unsigned long virt_addr=vma->vm_start;
    //         for(;virt_addr<=vma->vm_end;virt_addr+=PAGE_SIZE){
    //             struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
